...
<body style="padding-bottom: 30px;">
  ...
  <div>
    ...
    ?>
            ...
            <td><?=$row['stars'] . "/5"?></td> 
            <td>
              <form action="../controller/updateHomeReview.php" method="post">
                <input type="hidden" name="review-id" value="<?= $row['review_id'] ?>">
                <?php
                $is_displayed = $row['is_displayed'];
                echo "<input type='hidden' name='is-displayed' value='$is_displayed'>";

                if ($is_displayed == 0) {
                  echo "<button class='btn btn-primary' style='height:40px' onclick='return verifyReviewCountOverflow()'>Add</button>";
                } else {
                  echo "<button class='btn btn-secondary' style='height:40px' onclick='return verifyReviewCountUnderflow()'>Remove</button>";
                }
                ?>
              </form>
            </td>
  ...
  <!-- Control the no. of reviews added to/removed from home page -->
  <script>
    function verifyReviewCountOverflow() {
      <?php
      ...
      if ($result->num_rows == 4) {
        ?>
        alert('Cannot add more than 4 reviews to home page. Please remove existing reviews from the home page in order to add more.');
        return false;
    ...
    function verifyReviewCountUnderflow() {
      <?php
      $sql = "SELECT * FROM reviews WHERE is_displayed = 1";
      $result = $conn->query($sql);
      ...
      if ($result->num_rows <= 2) {
        ?>
        alert('Reviews in the home page cannot be less than 2. Please add more reviews to the home page before removing existing ones.');
        return false;
